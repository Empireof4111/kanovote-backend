...
export class AdminService {
  ...
  async getSupporterStats() {
    ...
    const growthRate =
      lastMonth > 0 ? Number((((thisMonth - lastMonth) / lastMonth) * 100).toFixed(2)) : 0;
  ...
  async getDashboardStats() {
    ...
    const registrationsByMonthMap = new Map<
      string,
      { month: string; registrations: number; target: number; sortKey: number }
    >();
    const registrationsByLgaMap = new Map<string, number>();
    const genderDistributionMap = new Map<string, number>();
    ...
    supporters.forEach((supporter) => {
      ...
      const gender =
        supporter.gender?.charAt(0).toUpperCase() + supporter.gender?.slice(1).toLowerCase();
      ...
      const range =
        age <= 25 ? '18-25' : age <= 35 ? '26-35' : age <= 45 ? '36-45' : age <= 55 ? '46-55' : '56+';
